...
#define GET(p, ind, shift)  ((uint32_t)p[ind] << shift)
...
static int
key_valid(unsigned char const *key, size_t n)
    {
    while (n--)
...
parse(struct vtag *s, char const * const data, size_t bytes)
    {
    unsigned char const *p = (unsigned char const *)data, *end = p + bytes;
    uint32_t len, to_do;
    ...
    if (p + len + 4 > end)
        return VE_CROPPED;
    if (!(s->encoder_string = strndup(p, len))
    ...
    while (to_do--) {
        ...
        switch (len) {
            ...
            default:
                ...
                char const *key = strlwr(strndup(p, sep - p));
                if (!key)
                    return VE_ALLOCATION;
                char const *value = strndup(sep + 1, len - (sep + 1 - p));
                if (!value)
                    {
                    free(key);
                    return VE_ALLOCATION;
                ...
                struct GSList *slist;
                g_hash_table_lookup_extended(s->hash_table, key, &orig_key, &slist);
                g_hash_table_steal(s->hash_table, key);
                if (orig_key)
                    free(orig_key);
                slist = g_slist_append(slist, value);
                g_hash_table_insert(s->hash_table, key, slist);
            }
        }
...
static void free_value(struct GSList *slist)
    {
...
static void slist_data_length(struct GSList slist, size_t *length)
    {
    *length += strlen(slist->data);
    }

char *
vtag_lookup(struct vtag *s, char const *key, enum vtag_lookup_mode, char *sep)
    {
    ...
    swich (vtag_lookup_mode) {
        ...
        case VLM_MERGE:
            if (!sep)
                sep = "";
            g_slist_foreach(slist, slist_data_length, &length);
            length += (g_slist_length(slist) - 1) * strlen(sep) + 1;
...
vtag_error_string(int error)
    ...
    switch ((enum vtag_error) error) {
        ...
        case VE_SHORT_COMMENT:
            return "comment too short to express key=value";
        case VE_MISSING_SEPARATOR:
            return "comment separator missing";
        case VE_MISSING_VALUE:
            return "comment value missing";
        case VE_INVALID_KEY:
            return "comment key contains illegal characters";
        default:
